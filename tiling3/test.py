...
while True:
  for theta in np.linspace(0,np.pi,13):
    for phi in np.arange(0,2*np.pi,1/(np.abs(r*np.cos(theta))+np.pi/3*1e-5)):
      ...
      bx.rotate(angle=vs.deg2rad(90)+phi,axis=bz.axis)
      by.rotate(angle=vs.deg2rad(90)+phi,axis=bz.axis)
      bz.rotate(angle=vs.deg2rad(90)+phi,axis=bz.axis)
      bx.rotate(angle=vs.deg2rad(180)+theta,axis=bx.axis)
      by.rotate(angle=vs.deg2rad(180)+theta,axis=bx.axis)
      bz.rotate(angle=vs.deg2rad(180)+theta,axis=bx.axis)
      bx.rotate(angle=vs.deg2rad(-90),axis=bz.axis)
      by.rotate(angle=vs.deg2rad(-90),axis=bz.axis)
      bz.rotate(angle=vs.deg2rad(-90),axis=bz.axis)
